(ns tesla-fsd.main-test
  "Contract + behavioral test for the tesla-fsd L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "tesla_fsd-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'tesla-fsd.main-test)]
