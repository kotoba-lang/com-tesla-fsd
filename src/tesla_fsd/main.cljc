(ns tesla-fsd.main
  "Kotodama WASM entrypoint for the Tesla_fsd clean-room actor (L4) — Clojure port.
  ...
  facts (`tesla_fsd.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "tesla_fsd")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"tesla_fsd.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "tesla_fsd-compat" :tier tier :entities entities} 200])
...
(defn start! [] :tesla_fsd-compat/ready)
